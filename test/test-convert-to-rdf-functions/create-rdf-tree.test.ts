...
test(
    "Test-toRdfSocialAgentRegistration-c4562da9", async () => {
        const expected = getExpectedRDFFromFile("agents/c4562da9SocialAgentRegistration/c4562da9.ttl")

        const actual = await new RdfFactory().create(socialAgentRegistrationc4562da9)
...
test(
    "Test-toRdfApplicationRegistration-2f2f3628", async () => {
        const expected = getExpectedRDFFromFile("agents/2f2f3628ApplicationRegistration/2f2f3628.ttl")

        const actual = await new RdfFactory().create(applicationRegistration2f2f3628)
...
test.each([
    ...
    const expected = getExpectedRDFFromFile(arg.expect_rdf_file_path)
...
test.each([
    ...
    const actual = await new RdfFactory().create(arg.instance, PREFIXEStest)
