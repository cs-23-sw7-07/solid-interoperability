...
test(
    "Test-toRdfSocialAgentRegistration-c4562da9", async () => {
        const expected = getRDFFromFile(join(PATH_TO_RDFS_EXAMPLES, "agents/c4562da9SocialAgentRegistration/c4562da9.ttl"))
...
test(
    "Test-toRdfApplicationRegistration-2f2f3628", async () => {
        const expected = getRDFFromFile(join(PATH_TO_RDFS_EXAMPLES, "agents/2f2f3628ApplicationRegistration/2f2f3628.ttl"))
...
test.each([
    ...
    const expected = getRDFFromFile(join(PATH_TO_RDFS_EXAMPLES, arg.expect_rdf_file_path))
