...
import {NotParsable, SAIViolationError} from "../../src";
...
describe('TurtleParser', () => {
    ...
    it('should be able to parse', async () => {
        ...
        store.add(triple(INTEROP + "grantedBy", "http://localhost:3000/Alice-pod/profile/card#me"));
        store.add(quad(namedNode("http://localhost:3000/Alice-pod/registries-unchangeable/agents/2f2f3628ApplicationRegistration/e2765d6dAccessGrant"), namedNode(INTEROP + "grantedAt"), literal("2020-12-04T20:15:47.000Z", namedNode("http://www.w3.org/2001/XMLSchema#dateTime"))));
    ...
    it('should not be able to parse an invalid turtle #1', async () => {
        await expect(() => parseTurtle("rdfTurtle")).rejects.toThrow(NotParsable);
    });
    ...
    it('should not be able to parse an invalid turtle #2', async () => {
        ...
        await expect(() => parseTurtle(text)).rejects.toThrow(NotParsable);
    });
