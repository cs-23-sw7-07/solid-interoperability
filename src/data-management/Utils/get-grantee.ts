...
export async function getAgent(rdf: Rdf, fetch: Fetch, predicate: string): Promise<Agent> {
    ...
    if (agentWebId) {
      ...
      if (isApplicationAgent(profile))
        return new ApplicationAgent(agentWebId);
      else
        return new SocialAgent(agentWebId);
    }
